...
class App(Gtk.Application, UserDataManager, BindingEditor):
	...
	def __init__(self, gladepath: str = "/usr/share/scc/glade",
	...
	def setup_widgets(self):
		...
		self.builder.add_from_file(Path(self.gladepath).joinpath("app.glade").as_posix())
		...
		self.lpad_test = Gtk.Image.new_from_file(Path(self.imagepath, "test-cursor.svg").as_posix())
		self.rpad_test = Gtk.Image.new_from_file(Path(self.imagepath, "test-cursor.svg").as_posix())
		self.stick_test = Gtk.Image.new_from_file(Path(self.imagepath, "test-cursor.svg").as_posix())
...
%s""") % (" ".join(shell_command), message), )
	...
	def generate_new_name(self):
		...
		filename = PurePath(get_profiles_path(), new_name + ".sccprofile")
	...
	def new_profile(self, profile: Profile, name: str):
		filename = PurePath(get_profiles_path(), name + ".sccprofile")
	...
	def on_btRenameProfile_clicked(self, *a):
		...
		old_fname = PurePath(get_profiles_path(), old_name + ".sccprofile")
		new_fname = PurePath(get_profiles_path(), new_name + ".sccprofile")
		try:
	...
	def on_mnuProfileDelete_activate(self, *a):
		...
		if d.run() == -5: # OK button, no idea where is this defined...
			fname = PurePath(get_profiles_path(), name + ".sccprofile")
			try:
				os.unlink(fname)
				try:
					os.unlink(fname + ".mod")
	...
	def set_daemon_status(self, status, daemon_runs):
		...
		icon = Path(self.imagepath, "scc-%s.svg" % (status,)).as_posix()
		print(icon)
		#exit()
	...
	def save_profile_selection(self, path):
		...
		open(PurePath(get_config_path(), self.CONFIG), "w").write(jstr)
	...
	def convert_old_profiles(self):
		...
		for name in os.listdir(get_profiles_path()):
			...
			try:
				...
				p.load(PurePath(get_profiles_path(), name))
			except:
